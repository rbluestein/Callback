...
Public Class Report
    ...
    Public Function Report(ByVal ErrorMessage As String, ByVal WriteToLog As Boolean, ByVal SendEmailPlease As Boolean, ByVal Shutdown As Boolean) As String
        ...
        Try
            ...
            If SendEmailPlease Then
                Coll.Add(Enviro.LogFileFullPath)
                Subj = "Callback error - User: " & IIf(cEnviro.LoggedInUserName = Nothing, cEnviro.LoggedInUserID, cEnviro.LoggedInUserName) & " - Time: " & Common.GetServerDateTime()
                SendEmailResults = SendEmail("HelpDesk@benefitvision.com;rbluestein@benefitvision.com", "automail@benefitvision.com", Nothing, Subj, ErrorMessage)
                EmailSuccess = SendEmailResults.Success
            End If

            If Shutdown And WriteToLog And EmailSuccess Then
                HeaderMessage = "An error has occurred requiring Callback to shut down. Callback has emailed a notice to the help desk. You may view the details of the problem in the log."
            ElseIf Shutdown And WriteToLog And (Not EmailSuccess) Then
                HeaderMessage = "An error has occurred requiring Callback to shut down. You may view the details of the problem in the log."
            ElseIf Shutdown And (Not WriteToLog) And EmailSuccess Then
                HeaderMessage = "An error has occurred requiring Callback to shut down. Callback has emailed a notice to the help desk."
            ElseIf Shutdown And (Not WriteToLog) And (Not EmailSuccess) Then
                HeaderMessage = "An error has occurred requiring Callback to shut down. "
            End If
    ...
    Public Shared Function SendEmail(ByVal SendTo As String, ByVal From As String, ByVal cc As String, ByVal Subject As String, ByVal TextBody As String, Optional ByRef AttachmentColl As Collection = Nothing) As Results
        ...
        '  Dim MMsg As System.Net.Mail.MailMessage
    ...
    Private Shared Function ReadLogFile() As String
        Dim StreamReader As System.IO.StreamReader = Nothing
        Dim FileText As String

        Try
            StreamReader = New System.IO.StreamReader(Enviro.LogFileFullPath)
    ...
    Public Sub WriteToLogFile(ByVal Message As String)
        ...
        Try
            ...
            Try
                StreamWriter = New System.IO.StreamWriter(Enviro.LogFileFullPath, True)
            Catch
