...
#Region " Page load "
    ...
    Private Sub PageLoad()
        ...
        Catch ex As Exception
            Dim ErrorObj As New ErrorObj("Error #102: Index PageLoad. " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub LoadEnviro(ByVal AltLoginInd As Boolean, ByVal AltLogin As String)
        ...
        Try
            ...
            'For i = 0 To Request.QueryString.Keys.Count - 1
            '    Key = Request.QueryString.GetKey(i)
            '    KeyMatch = System.Text.RegularExpressions.Regex.Match(Key, DVRStat)

            '    Value = Request.QueryString.Item(Request.QueryString.GetKey(i))
            '    ValueMatch = System.Text.RegularExpressions.Regex.Match(Value, DVRStat)

            '    If KeyMatch.Success OrElse ValueMatch.Success Then
            ...
            If Request.QueryString.Keys.Count = 1 Then
                ...
                Value = Request.QueryString.Item(Request.QueryString.GetKey(0))
                If Key = "Source" AndAlso Value = "DetectCaller" Then
                ...
                Case PageMode.Postback
                    ...
                    Select Case Request.Form("hdAction")
                        ...
                        Case "ApplyFilter"
                            ...
                            cIndexSess.Filter.EmpName = Request.Form("txtEmpName")
...
#Region " Display Page "
    ...
    'Private Function OldGetQueryInfo(ByVal InfoType As String, ByVal DG As DG, ByVal OrderByType As DG.OrderByType) As Collection
    '    Dim i As Integer
    '    Dim sb As New System.Text.StringBuilder
    '    Dim Sql As String
    '    Dim dt As DataTable = Nothing
    ...
    '            sb.Append("DaysRemaining = case ")
    '            sb.Append("when DateDiff(d, getDate(), ISNULL(dbo.ufn_GetEnrollWinEndDate(cm.EmpID, cm.ClientID), '1/1/2010')) < 0 then 'No win' ")
    '            sb.Append("else Cast((SELECT DateDiff(d, getDate(), dbo.ufn_GetEnrollWinEndDate(cm.EmpID, cm.ClientID))) as varchar(10))  ")
    '            sb.Append("end, ")
    ...
    '        'DG.GenerateSQL(Sql, ShowFilter, NonFilterWhereClause, OrderByType, Request, cIndexSess.DGSess.FilterOnOffState, Request.Form("hdFilterShowHideToggle"))
    ...
    Private Function GetQueryInfo(ByVal InfoType As String, ByRef DG As DG, ByVal OrderByType As DG.OrderByType) As Collection
        ...
        Dim sbParams As System.Text.StringBuilder
        ...
        Try
            ...
            'DG.GenerateSQL(OrderByType, OrderByStr, ShowFilter, NonFilterWhereClause, OrderByType, Request, cIndexSess.DGSess.FilterOnOffState, Request.Form("hdFilterShowHideToggle"), QueryColl)
            OrderByStr = DG.SetDisplayProperties(OrderByType, cIndexSess.DGSess.FilterOnOffState, Request.Form("hdFilterShowHideToggle"))
            sbParams = GetParams(OrderByStr)
            ...
            Select Case InfoType
                Case "RecordCount"
                    If sbParams.Length = 0 Then
                        QueryPack = Common.GetDTWithQueryPack("exec usp_GetCallbackIndex @Recordcount=1")
                    Else
                        QueryPack = Common.GetDTWithQueryPack("exec usp_GetCallbackIndex @Recordcount=1, " & sbParams.ToString)
                    End If
                    Recordcount = QueryPack.dt.Rows(0)(0)
                    Coll.Add(Recordcount, "RecordCount")
                Case "Data"
                    If sbParams.Length = 0 Then
                        QueryPack = Common.GetDTWithQueryPack("exec usp_GetCallbackIndex @Recordcount=0")
                    Else
                        QueryPack = Common.GetDTWithQueryPack("exec usp_GetCallbackIndex @Recordcount=0, " & sbParams.ToString)
                    End If
                    dt = DBase.GetDTExtended(QueryPack.dt)
                    Coll.Add(dt, "Data")
                    If DG.GetTotal IsNot Nothing Then
                        For i = 0 To dt.Rows.Count - 1
                    ...
                    End If
                Case "Sql"
                    QueryPack = Common.GetDTWithQueryPack("exec usp_TempTemp @InfoType='Sql' " & sbParams.ToString)
            End Select
    ...
    Private Function GetParams(ByVal OrderByStr As String) As System.Text.StringBuilder
        ...
        Try
            ...
            FilterColl.Assign("@OrderBy", OrderByStr)
            ...
            If Common.IsNotBlank(FilterValue) Then
                ...
                If Box.GetUpperBound(0) = 0 Then
                    FilterColl.Assign("@EmpLastName", Common.StrOutHandler(Box(0), False, StringTreatEnum.SideQts_SecApost))
                Else
                    FilterColl.Assign("@EmpLastName", Common.StrOutHandler(Box(0), False, StringTreatEnum.SideQts_SecApost))
                    FilterColl.Assign("@EmpFirstName", Common.StrOutHandler(Box(1), False, StringTreatEnum.SideQts_SecApost))
                End If
            End If
            ...
            If IsNumeric(Box(0)) AndAlso IsNumeric(Box(1)) AndAlso IsNumeric(Box(2)) Then
                FilterColl.Assign("@CreationDateFrom", Common.StrOutHandler(Box(1) & "/" & Box(2) & "/" & Box(0), False, StringTreatEnum.SideQts))
            End If
            If IsNumeric(Box(3)) AndAlso IsNumeric(Box(4)) AndAlso IsNumeric(Box(5)) Then
                FilterColl.Assign("@CreationDateTo", Common.StrOutHandler(Box(4) & "/" & Box(5) & "/" & Box(3), False, StringTreatEnum.SideQts))
            End If

            FilterValue = cIndexSess.Filter.ClientID
            If Common.IsNotBlank(FilterValue) Then
                FilterColl.Assign("@ClientID", Common.StrOutHandler(FilterValue, False, StringTreatEnum.SideQts))
            End If

            FilterValue = cIndexSess.Filter.State
            If Common.IsNotBlank(FilterValue) Then
                FilterColl.Assign("@State", Common.StrOutHandler(FilterValue, False, StringTreatEnum.SideQts))
            End If
            ...
            For i = 1 To FilterColl.Count
                If Not IsDBNull(FilterColl(i)) Then
                    If NumItems = 0 Then
                        sbParams.Append(FilterColl.Key(i) & "=" & FilterColl(i))
                        NumItems += 1
                    Else
                        sbParams.Append(", " & FilterColl.Key(i) & "=" & FilterColl(i))
                        NumItems += 1
        ...
        Catch ex As Exception
            Throw New Exception("Error #119: Index GetParams. " & ex.Message, ex)
        End Try
    End Function
...
#End Region
    ...
    Public Shared Function OldCheckoutRecordIfAvailable(ByVal LoggedInUserID As String, ByVal CallbackID As Int64, ByVal CallbackAttemptID As Int64, ByVal PageCode As String, ByVal MaintDelete As String, ByVal NewExisting As String, ByVal MaintOpen As String, ByVal AttemptOpen As String) As String
        ...
        Try
            ...
            Sql.Append("INNER JOIN UserManagement..Users u ON ")
            Sql.Append("cm.CallbackID = " & CallbackID & " ")
            Sql.Append("AND u.UserID = cm.CheckoutUserID ")
            Sql.Append("AND u.UserID <> '" & mEnviro.LoggedInUserID & "'")
    ...
    <System.Web.Services.WebMethod()>
    Public Shared Function IntermediateCheckoutRecordIfAvailable(ByVal LoggedInUserID As String, ByVal CallbackID As Int64, ByVal CallbackAttemptID As Int64, ByVal PageCode As String, ByVal MaintDelete As String, ByVal NewExisting As String, ByVal MaintOpen As String, ByVal AttemptOpen As String) As String
        Dim mEnviro As Enviro
        ' Dim Sql As New System.Text.StringBuilder
        Dim Querypack As DBase.QueryPack
        Dim dr As DataRow = Nothing
        ...
        Try
            ...
            mEnviro = Common.GetSessionObj("Enviro")
            Sql = "exec usp_TryRecordCheckout @CallbackID=" & CallbackID & ", @LoggedInUserID='" & mEnviro.LoggedInUserID & "'"
            Querypack = Common.GetDTWithQueryPack(Sql)
    ...
    ' from v2
    <System.Web.Services.WebMethod()>
    Public Shared Function CheckoutRecordIfAvailable(ByVal LoggedInUserID As String, ByVal CallbackID As Int64,
    ByVal CallbackAttemptID As Int64, ByVal PageCode As String, ByVal MaintDelete As String, ByVal NewExisting As String,
    ByVal MaintOpen As String, ByVal AttemptOpen As String) As String
        Dim mEnviro As Enviro
        Dim Querypack As DBase.QueryPack
        Dim dr As DataRow = Nothing
        Dim Results As New System.Text.StringBuilder
        Dim IsAlreadyCheckedOut As Char
        Dim CheckoutUser As String = String.Empty
        Dim CheckoutUserID As String = String.Empty
        Dim ErrorInd As String = String.Empty
        Dim ErrorMessage As String = String.Empty
        Dim NewCheckoutSuccessfulInd As String = String.Empty
        Dim Sql As String
        ...
        Try
            ...
            If Querypack.Success Then
                ErrorInd = "0"
                If Querypack.dt.Rows(0)("CheckoutSuccess") Then
                    IsAlreadyCheckedOut = "0"
                    NewCheckoutSuccessfulInd = "1"
                Else
                    IsAlreadyCheckedOut = "1"
                    NewCheckoutSuccessfulInd = "0"
                    CheckoutUser = Querypack.dt.Rows(0)("CurrentCheckoutUser")
                End If
            Else
                ErrorInd = "1"
                ErrorMessage = Common.ToJSAlert(Querypack.TechErrMsg)
